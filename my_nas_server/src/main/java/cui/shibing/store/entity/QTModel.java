...
public class QTModel extends com.querydsl.sql.RelationalPathBase<TModel> {
    ...
    public final NumberPath<Byte> valid = createNumber("valid", Byte.class);
    ...
    public void addMetadata() {
        addMetadata(attributions, ColumnMetadata.named("attributions").withIndex(4).ofType(Types.LONGVARCHAR).withSize(1073741824).notNull());
        addMetadata(ctime, ColumnMetadata.named("ctime").withIndex(5).ofType(Types.BIGINT).withSize(19).notNull());
        addMetadata(id, ColumnMetadata.named("id").withIndex(1).ofType(Types.BIGINT).withSize(19).notNull());
        addMetadata(identifier, ColumnMetadata.named("identifier").withIndex(2).ofType(Types.VARCHAR).withSize(256).notNull());
        addMetadata(modelName, ColumnMetadata.named("model_name").withIndex(3).ofType(Types.VARCHAR).withSize(256).notNull());
        addMetadata(utime, ColumnMetadata.named("utime").withIndex(6).ofType(Types.BIGINT).withSize(19).notNull());
        addMetadata(valid, ColumnMetadata.named("valid").withIndex(7).ofType(Types.TINYINT).withSize(3).notNull());
    }
